...
\end{split}
\end{equation}
Using the facts that: (1) $A_i - \lambda_i$ is Hermitian (2) $\ip{v}{v} = |v|^2$ for any vector $v \in \mathbb{C}^n$ we find that:
\begin{equation}
\begin{split}
...
We will use this advantage to study the robustness of waves on disorded media in Sections \ref{sec:px}-\ref{sec:prop_px}.
...
\section{Propagation of States Along Index Boundary in the Haldane Model} \label{sec:prop_Hald}
...
\section{\texorpdfstring{$p_x + ip_y$}{px + ipy} Model} \label{sec:px}
...
It is unclear how to generalize the Haldane model to disordered media, specifically because it is unclear how to generalize the concept of nearest-neighbor and next-nearest neighbor sites to this case.
...
\end{equation}
This can only occur if $m = n = 1$ and $\mu' = 2$, if $m \neq n$ and $\mu' = 0$, or if $m = n = 0$ and $\mu' = - 2$. These are the only values of $\mu'$ such that a topological transition (a change in the Chern number) can happen. So for example, if $t = 1$, a topological transition can only occur at $\mu = 0$ or $\pm 4$. When we computed the Clifford index of a finite system numerically we found that the results agreed with these observations (see Figure \ref{fig:pxipy_params}). These numerics show how to choose the parameters of the model in such a way that we expect to see edge states. We will study this in the following section.

\begin{figure}
\centering
\includegraphics[width=.9\textwidth]{figures/phase_diagram.png}
\caption{Clifford index in $p_x + ip_y$ model at $\lambda_3 = 0$ with varying parameters: we fix $t = 1$ and let $\mu$ and $\Delta$ vary, computing the index of a point in the center of the lattice.
...
The fact that we see very similar behavior in this model is significant and supports the idea that the Clifford index can be thought of as a suitable generalization of the Chern number to structures which are not periodic.
...
\section{Propagation in \texorpdfstring{$p_x + ip_y$}{px + ipy} Model} \label{sec:prop_px}
...
When we attempted to see propagation of states around interfaces in the interior of the medium, we saw that they dispersed very quickly throughout the medium, so it appears this propagation is not protected (See Figure \ref{fig:mu_dis_interior_prop}).
